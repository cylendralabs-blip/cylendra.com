 ...
 * 
 * هذا الملف محاكاة لـ Strategy Service
...
export class StrategyServiceMock {
  static async createStrategy(_strategy: Partial<BaseStrategy>): Promise<BaseStrategy> {
    console.warn('[MOCK] Strategy creation disabled - strategy_templates table does not exist');
  ...
  static async updateStrategy(_id: string, _updates: Partial<BaseStrategy>): Promise<BaseStrategy> {
    console.warn('[MOCK] Strategy update disabled - strategy_templates table does not exist');
    throw new Error('Strategy templates feature not available');
  }

  static async deleteStrategy(_id: string): Promise<void> {
    console.warn('[MOCK] Strategy deletion disabled - strategy_templates table does not exist');
  }

  static async getStrategy(_id: string): Promise<BaseStrategy | null> {
    console.warn('[MOCK] Strategy retrieval disabled - strategy_templates table does not exist');
    return null;
  }

  static async getUserStrategies(_userId: string, _type?: StrategyType): Promise<BaseStrategy[]> {
    console.warn('[MOCK] Get user strategies disabled - strategy_templates table does not exist');
    return [];
  }

  static async toggleStrategyStatus(_id: string, _isActive: boolean): Promise<void> {
    console.warn('[MOCK] Toggle strategy status disabled - strategy_templates table does not exist');
  }

  static async cloneStrategy(_id: string, _newName: string): Promise<BaseStrategy> {
    console.warn('[MOCK] Clone strategy disabled - strategy_templates table does not exist');
    throw new Error('Strategy templates feature not available');
  }
  ...
  static async backtest(_strategyId: string, params: {
    ...
    console.warn('[MOCK] Backtest disabled - strategy_templates table does not exist');
...
export default StrategyServiceMock;
