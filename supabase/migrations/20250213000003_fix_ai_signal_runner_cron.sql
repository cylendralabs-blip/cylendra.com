-- Fix ai-signal-runner cron job
-- This migration fixes the "unrecognized configuration parameter" error
-- by recreating the cron job with the correct configuration
...
-- Recreate ai-signal-runner cron job with correct configuration
-- Using hardcoded values directly in the cron block (no variables)
DO $$
BEGIN
  -- Schedule the job with explicit hardcoded values (no configuration parameters or variables)
  ...
  RAISE NOTICE '✅ Cron job "ai-signal-runner" recreated successfully (every 10 minutes)';
EXCEPTION WHEN OTHERS THEN
